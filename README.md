...
| `capsim serve` | Control-plane service on `127.0.0.1:8321`: run lifecycle over HTTP (`/api/runs`, `/api/status`, `/api/export`, `/api/doctor`, catalogs) and live telemetry over WebSocket (`/ws/telemetry` — run/snapshot/turn/step events). One run at a time, in-process; `run.db` stays the source of truth. The Phase 3 UI mounts here. |
