...
export function SmbOnPremProvider({ children }: { children: ReactNode }) {
  const [sizingData, setSizingData] = useState<SizingData | null>(null);
  useEffect(() => {
    let cancelled = false;
    fetch('/Intel_sizing_qwen3.json')
      .then(r => r.json())
      .then((d: SizingData) => { if (!cancelled) setSizingData(d); })
      .catch(err => console.error('Failed to load Intel_sizing_qwen3.json', err));
...
export function useSmbOnPrem(): State {
  const v = useContext(Ctx);
  if (!v) throw new Error('useSmbOnPrem must be used within SmbOnPremProvider');
  return v;
}