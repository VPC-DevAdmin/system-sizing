...
export default function TpsCurveChart({ cohort, height = 300 }: Props) {
  ...
  const raw = sorted
    .filter(c => c.tpot_p50_ms > 0)
    .map(c => ({
      x: c.pool_size,
      p50: 1000 / c.tpot_p50_ms,
    }))
    .filter(p => p.p50 <= 50);

  // Plot the measured p50 directly so every cohort renders a continuous line
  // through its actual datapoints (smoothing previously caused gaps on
  // cohorts where the smoothed series clipped the visible range).
