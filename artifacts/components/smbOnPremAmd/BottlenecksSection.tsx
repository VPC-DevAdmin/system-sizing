...
// timeline seconds. When set, the chart shows only that window with the x-axis
// zeroed at the window start.
const WINDOW_OVERRIDES: Record<string, [number, number]> = {
  'chat_heavy:16': [165, 285],
  'chat_heavy:32': [24, 144],
  'chat_heavy:64': [15, 135],
  'general_knowledge:16': [300, 420],
  'general_knowledge:32': [200, 320],
  'general_knowledge:64': [64, 184],
  'writer_dominant:16': [363, 483],
  'writer_dominant:32': [50, 170],
  'writer_dominant:64': [130, 250],
  'software_engineering:16': [345, 465],
  'software_engineering:32': [404, 524],
  'software_engineering:64': [60, 180],
  'analyst_team:16': [210, 330],
  'analyst_team:32': [105, 225],
};
...
function buildSeries(cohort: RawCohort, pool: number): SeriesResult {
  ...
  // Layer mapping (per spec):
  //   Prefill       = prefill column
  //   Decode        = decode column (focal point)
  //   Read/think    = think column
  //   Transitioning = pool - (prefill + decode + think)   [the remainder, incl. idle]
  const raw = tl.rows
    .map(r => {
      const prefill = r[iP] ?? 0;
      const decode = r[iD] ?? 0;
      const think = r[iTh] ?? 0;
      const engaged = prefill + decode + think;
      const transitioning = Math.max(0, pool - engaged);
      return { t: r[iT], prefill, decode, read_think: think, transitioning, totalEngaged: engaged };
    })
    .sort((a, b) => a.t - b.t);

  // When a window override exists, filter to that absolute timeline window
  // and re-zero the x-axis at the window start. Otherwise show the full timeline.
  const cohortId = (cohort as unknown as { id: string }).id;
  const override = WINDOW_OVERRIDES[`${cohortId}:${pool}`];
  const win = override
    ? raw.filter(r => r.t >= override[0] && r.t <= override[1])
    : raw;
...
function hardwareDescriptor(freqMin: number): string {
  ...
  return 'The chip ran at or near its sustainable CPU-decode floor, the hardware throughput envelope for this workload class.';
}
        ...
        <div style={{ height: 360 }}>
          <ResponsiveContainer>
            <ComposedChart data={data} margin={{ top: 16, right: 24, left: 8, bottom: 28 }}>
              ...
              <Area type="monotone" dataKey="decode"        name="Decode"        stackId="1" stroke="hsl(var(--primary))"                 fill="hsl(var(--primary) / 0.95)" isAnimationActive />
...
export default function BottlenecksSection() {
  ...
  const { cohortById } = useSmbOnPremAmd();
