...
// AMD-specific variant that mirrors TpsCurveChart's fallback: when
// tpot_p50_ms is 0 (throughput collapsed below the measurable floor),
// treat the point as 1 tok/s so the supported-team-size card uses the
// same extrapolated point that's drawn on the chart.
function poolSizeAtTpsWithFallback(cohort: RawCohort, minTps: number): number {
  const pts = cohort.curve
    .filter(c => c.pool_size > 0 && (c.tpot_p50_ms > 0 || c.ttft_p95_ms > 0))
    .map(c => ({ pool: c.pool_size, tps: c.tpot_p50_ms > 0 ? 1000 / c.tpot_p50_ms : 1 }))
...
export default function DemandVsCapacity() {
  ...
  const cards = COHORTS.map(copy => {
    ...
    return {
      copy,
      comfortable: poolSizeAtTpsWithFallback(data, 10),
      acceptable: poolSizeAtTpsWithFallback(data, 4),
    };
