// Static config for /demos/smb-on-prem-amd — everything not in AMD_sizing_qwen3.json
...
export const COHORTS: CohortCopy[] = [
  { id: 'chat_heavy',           name: 'Customer support team', description: 'Quick-lookup heavy. High request rate per active user.', densityCoefficient: 1.4, defaultMixPct: 15, status: 'tight',
    ...
    recommendation: 'Output volume is what limits this team. The Higher Capacity on R7625 profile or splitting the team onto a second R7625 node gives the most room.' },
  ...
  { id: 'writer_dominant',      name: 'Marketing / content team', description: 'Drafter-heavy. Output-bound work pressures the decode pipeline.', densityCoefficient: 1.0, defaultMixPct: 10, status: 'tight',
    ...
    recommendation: 'Similar pattern to support: a top-bin EPYC SKU helps most. Consider the Higher Capacity on R7625 profile if writers run hot all day.' },
  { id: 'software_engineering', name: 'Software engineering team', description: 'Code-assist heavy. Long generated outputs sustain decode pressure.', densityCoefficient: 1.2, defaultMixPct: 15, status: 'at_capacity',
    ...
    recommendation: 'Tightest envelope of the five. For >50 active engineers, plan a second R7625 node or step up to the Higher Core Count on R7625 profile.' },
...
export const TEST_BENCH: Array<[string, string]> = [
  ...
  ['CPU',              '2× AMD EPYC 9374F (Zen 4, 32C / 64T per socket)'],
  ['Cores / threads',  '64 / 128'],
  ['Memory',           '1 TB DDR5'],
  ['NUMA',             'Two nodes, two sockets'],
  ['GPU',              'None'],
  ['OS',               'Ubuntu 24.04'],
];

export const SOFTWARE_STACK: Array<[string, string]> = [
  ['Inference engine',       'vLLM (dual-socket) on CPU'],
  ['Attention backend',      'torch_native'],
  ['Model',                  'Qwen3-30B-A3B-Instruct (FP8)'],
  ['KV cache',               '64 GB allocated'],
  ['Max model length',       '8,192 tokens'],
  ['Max total tokens',       '131,072'],
  ['CPU bind',               'cores 0–63 across 2 sockets'],
...
export const CONFIG_PROFILES: ConfigProfile[] = [
  { id: 'entry',     name: 'Entry',                          bestFit: '100–200 users',      cpu: 'Single EPYC 9004 24–32C',         memory: '256 GB DDR5',  sockets: 1, costMin: 18000, costMax: 22000, capacityEnvelope: '~5–10 Active',   capacityMultiplier: 0.55 },
  { id: 'standard',  name: 'Standard (tested)',              bestFit: '500–800 users',      cpu: '2× EPYC 9374F (32C)',             memory: '1 TB DDR5',    sockets: 2, costMin: 38000, costMax: 42000, capacityEnvelope: '~20–30 Active',  capacityMultiplier: 1.0, isTested: true },
  { id: 'capacity',  name: 'Higher Capacity on R7625',       bestFit: '700–1,000 users',    cpu: '2× EPYC 9474F / 9554 top-bin',    memory: '1.5 TB DDR5',  sockets: 2, costMin: 50000, costMax: 60000, capacityEnvelope: '~1.2× standard', capacityMultiplier: 1.2 },
  { id: 'dual',      name: 'Higher Core Count on R7625',     bestFit: '1,000–1,500 users',  cpu: '2× EPYC 9654 (96C)',              memory: '1–2 TB DDR5',  sockets: 2, costMin: 60000, costMax: 75000, capacityEnvelope: '~1.8× standard', capacityMultiplier: 1.8 },
];
...
export const CONFIG_LEVERS: ConfigLever[] = [
  { rank: 1, name: 'CPU frequency',  description: 'Decode-bound workloads spend most of their time waiting on per-core compute. The 9374F\'s high all-core boost is what makes it the decode champion in the EPYC line — TPOT falls and the SLA ceiling rises directly with frequency.', impact: '+15–25% capacity' },
  { rank: 2, name: 'Socket / node count', description: 'The R7625 is already dual-socket. The next step up is either a denser SKU (more cores per socket) or a second R7625 node behind a load balancer.',                                                                              impact: '~1.8× capacity, +40% cost' },
  ...
  { rank: 4, name: 'Memory speed',   description: 'For this MoE model on EPYC Genoa, memory bandwidth is not the binding constraint. DDR5-4800 vs DDR5-5600 is a wash.',                                                                                                                impact: 'Minimal impact for this workload' },
];
...
export const DELL_CTA_URL =
  'https://www.dell.com/en-us/shop/dell-poweredge-servers/poweredge-r7625-rack-server/spd/poweredge-r7625/pe_r7625_15972_vi_vp';
